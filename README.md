...
강서구 빅데이터 활용 공모전의 주제는 개방된 데이터를 활용하여 주민의 생활불편 해결 성공사례 및 공공 이익·발전에 기여할 수 있는 자유주제이다.
...
## 데이터 확인 및 전처리
+ 사용한 모든 데이터는 공공데이터 포탈에서 수집하였다. 대부분의 데이터에 결측치가 존재하지 않았다.
+ 또한 프로젝트에 데이터 전처리가 필요한 구간이 많지 않았다.
...
## 힘들었던점
+ 가장 힘들었던 점은 주제 정하기였다. 약 4주정도 프로젝트를 진행한것 같은데, 2주동안 주제를 정하지 못했다.
+ 그 이유는 여러가지가 있었는데,
  1. 팀원들이 강서구 주민이 아니어서 강서구에 필요한 것이 무엇인지 알 수 없었다.
  2. 공공 이익에 도움이 되는 행정 정책이나 현재 시행되고있는 공공 서비스를 잘 알지 못했다.
  3. 가장 큰 부분은 문제정의인데, 문제를 정의해야 해결이 가능했지만 문제 자체를 인식하지도 못하였다.
+ 주제를 정한 이후에도 방향을 잡기가 어려웠다. 활용할 데이터를 찾는 것 또한 어려웠다. 어떤 데이터를 사용해야하는지는 알았지만 우리가 필요한 데이터를 찾아오는 것이 힘들었다. 공공데이터, 그중 우리가 사용하는 데이터의 특성상 행정적인 정보가 많이 들어가있었고, 이 정보들은 그닥 유용하지 않았다. (예를들면 학교코드와 같이 임의로 부여한 값)
+ 다른 프로젝트를 진행하게 된다면, 공공기관에서 주최하는 프로젝트는 하지 않을 것 같다.
